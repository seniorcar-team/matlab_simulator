function [] = generate_look_up_table_consider_a_and_j()
...
for iv = 1:3
    ...
    vel_str = num2str(velocity(17-iv)*10);%一時的に変更、もとは17
    for ik0 = 1:13
        counter = 0;
        ...
        file_name = ['look_up_table_a_and_j_',vel_str,'_',k0_str];
...
function last_state = generate_last_state(p1,p2,p3,k0,x)%p1:加速度，p2:中間操舵角，p3:終端操舵角
    path_array = motion_model_base_trajectory_genarator_consider_a_and_j(p1,p2,p3,k0,x);%加速度，中間地点の操舵角，目標操舵角，初期操舵角
    path_length = size(path_array,1);
