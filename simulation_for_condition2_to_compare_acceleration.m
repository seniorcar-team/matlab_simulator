function acceleration_array = simulation_for_condition2_to_compare_acceleration(Pedestrian,file_num)
%% simulationの実行
...
global road_width; road_width = 10.0;%幅10mの道
...
%global pedestrian_width; pedestrian_width = 200*10^-3;%歩行者幅[m]
...
result.x=zeros(1000,7); %車両状態Xの格納+フラッグと衝突判定の記録　（5+2）
result.ped = zeros(length(Pedestrian_ver3),1000,2);
result.ped_speed = zeros(length(Pedestrian_ver3),1000);
result.ped_acceleration = zeros(length(Pedestrian_ver3),1000);
...
result.x(1,:)=[x',3,1];
result.ped(num,1,1) = Pedestrian_ver3(num).tmp_position(1);
result.ped(num,1,2) = Pedestrian_ver3(num).tmp_position(2);
result.ped_speed(num,1) = norm(Pedestrian_ver3(num).velocity);
result.ped_position_error(num,1) = norm(Pedestrian_ver3(num).tmp_position-predict_obserbable_pedestrian_position(num,5:6));
...
%                    elseif ttc<time_to_goal && ttc < 1.35
    ...
    %何秒分のパスが引けるか
%変更部分　surfaceの方に入っているlookuptableは負担を考慮していないもの
        path_array = state_lattice_motion_planner_for_c2_c4(x,existence_prohability_distrubution_all(:,:,:),goal,predict_obserbable_pedestrian_position,i,Pedestrian_ver3);
%変更部分
        path_start_time = i;
...
%    x =  f_sim_new(x,u);
    ...
    result.x(i+1,6) = 1*cruising_flag+3*following_flag+4*stop_flag+5*danger_flag;%ちょっとややこしくなってる
    result.x(i+1,7) = is_collision_now;
        %シミュレーション結果の保存
 
    for num = 1:length(Pedestrian_ver3)
        result.ped(num,i+1,1) = Pedestrian_ver3(num).tmp_position(1);
        result.ped(num,i+1,2) = Pedestrian_ver3(num).tmp_position(2);
        result.ped_speed(num,i+1) = norm(Pedestrian_ver3(num).velocity);
        result.ped_acceleration(num,i+1)= (result.ped_speed(num,i+1) - result.ped_speed(num,i))/0.1;
        result.ped_position_error(num,i+1) = norm(Pedestrian_ver3(num).tmp_position-predict_obserbable_pedestrian_position(num,5:6));
    ...
    if x(4) == 0
        ...
        if i-stop_counter>1200%速度が0になってからこの処理に入り，この処理の前でstop_counterに代入を行うので，stop_counterが0のままこの処理に入ってすぐプログラムが止まることはないはず
            ...
            result.x(i+2:1000,:)=[];
            result.ped(:,i+2:1000,:) = [];
            result.ped_speed(:,i+2:1000) = [];
            result.ped_position_error(:,i+2:1000) = [];
            loop_finish_num = i;
            is_collision_now = Judgement_collision(x,Pedestrian_ver3);
            break;
        end
...
file_name_first_position_ped = ['pedestrian_first_state_condition2_',file_num_str];
...
lateral_acceleration_array = abs(result.x(:,4).^2./(vehicle_front_length).*tan(result.x(:,5)));
B = find(result.x(:,6)==1);
lateral_acceleration_array_2 = lateral_acceleration_array(B);
acceleration_array = zeros(1,2);
if ~isempty(lateral_acceleration_array_2)
    acceleration_array(1) = max(lateral_acceleration_array_2);
    min_ped_acceleration = min(min(result.ped_acceleration));
    max_ped_acceleration = max(max(result.ped_acceleration));
    acceleration_array(2) = max_ped_acceleration - min_ped_acceleration;
end
filename = ['seniorcar_state_condition2_',file_num_str];
