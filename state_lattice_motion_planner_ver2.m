...
file_name = ['look_up_table_a_and_j_',vel_str,'_',steer_str];
...
lookuptable_plus = zeros(table_length,4);
...
for itable = 1:table_length
    ...
    if plus_min_direc<candidate_state_direc || -minus_min_direc>candidate_state_direc
        lookuptable(itable,12) = (lookuptable(itable,9)/max_speed_eval...
            +lookuptable(itable,10)/max_direction_eval...
            +1.5*lookuptable(itable,11)/max_direc_eval)/2;
    else
        lookuptable(itable,12) = lookuptable(itable,9)/max_speed_eval...
            +lookuptable(itable,10)/max_direction_eval...
            +1.5*lookuptable(itable,11)/max_direc_eval;
    end
end

lookuptable = sortrows(lookuptable,12,'descend');
%lookuptable(1,:)
%disp(lookuptable);
...
for itable = 1:table_length
    ...
    path_array_pre = optimize_trajectory(r_candidate_point,x(5),lookuptable(itable,6:9),[0,0,0,x(4),x(5)]);
...
function last_state = generate_last_state(p1,p2,p3,k0,x)%p1:加速度，p2:中間操舵角，p3:終端操舵角
    path_array = motion_model_base_trajectory_genarator_new(p1,p2,p3,k0,x);%加速度，中間地点の操舵角，目標操舵角，初期操舵角
