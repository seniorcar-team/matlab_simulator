function emergency_array = simulation_for_condition4(Pedestrian,file_num)
...
for i=1:1000
    ...
    %存在確率分布のマップに対してのパス形成
        ...
        path_array = state_lattice_motion_planner_forc2_c4(x,existence_prohability_distrubution_all(:,:,:),goal,predict_obserbable_pedestrian_position,i,Pedestrian_ver3);%現在時刻から，2秒先までの確率分布を用いる
    %何秒分のパスが引けるか
...
L = [];
N = [];
file_name_first_position_ped = ['pedestrian_first_state_condition4_',file_num_str,'_',dd];
...
filename = ['seniorcar_state_condition4_',file_num_str,'_',dd];
